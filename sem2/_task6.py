...
class Bank:
    ...
    def _in(self, cash: int) -> int:
        self._BALANCE += cash
        return self._BALANCE

    def _out(self, cash: int, commission: int) -> int | None:
    ...
    def _check_operation(self) -> int:
        self._COUNTER += 1
        cash = defs.get_natural_int_between_number(
            f"Введите сумму от {self._MIN_CASH:_} до {self._MAX_CASH:_}: ",
            self._MIN_CASH,
            self._MAX_CASH,
        )
    ...
    def start(self):
        ...
        print(
            f"Сумма пополнения и снятия кратны {self._MIN_CASH:_} у.е.\n"
