...
def sum_fraction(array_1: list[int, int], array_2: list[int, int]) -> list[int, int]:
    ...
    '''
    result = [array_1[0] * array_2[1] + array_2[0] * array_1[1], array_1[1] * array_2[1]]
    return cut_fraction(result)


def multypl_fraction(array_1: list[int, int], array_2: list[int, int]) -> list[int, int]:
    '''
...
def print_answer(array_1: list[int, int], array_2: list[int, int], symbol: str, operation) -> print:
    '''
    Выводит на экран пример с двумя дробями и результат операции
    '''
    print(f'{print_fraction(array_1)} {symbol} {print_fraction(array_2)} = {print_fraction(operation)}')


print('Программа просит ввести 2 дроби вида a/b. Затем выводит сумму и произведение этих дробей.')
