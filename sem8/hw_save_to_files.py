...
def get_result_travers_directory(directory: Path, result={}) -> dict[int:{str: str | int}]:
    '''
    Обходит директорию и выдает список фалов и папок в ней и всех вложенных папках
    '''
    for path, directories, files in os.walk(directory):
        for file in files:
            result[len(result) + 1] = {'name': file, 'parent': path.split('\\')[-1], 'type': 'file', 'size': f'{os.path.getsize(os.path.join(path, file))} bytes'}
        for direct in directories:
            result[len(result) + 1] = {'name': direct, 'parent': path.split('\\')[-1], 'type': 'folder', 'size': f'{volume_path(os.path.join(path, direct))} bytes'}
